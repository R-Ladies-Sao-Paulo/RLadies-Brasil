...
RLadies.Brasil::eventos_recentes_brasil %>% 
  ...
  dplyr::mutate(title = stringr::str_remove_all(title, ", Brazil, Latin America"),
                data = lubridate::as_date(start),
                data = format(data, "%d/%b/%Y", locale = "pt_BR")) %>% 
  dplyr::select(group, title, data, url) %>%
  knitr::kable(col.names = c("Capítulo", "Nome do evento", "Data", "Link"))
  
```

Dados de eventos recentes obtidos no [calendário do R Community Explorer](https://r-community.github.io/event-explorer/rcalendar.html) (Thanks to [@Benubah](https://github.com/benubah) for [this suggestion](https://github.com/rladies/meetupr/issues/79#issuecomment-743910328))!
